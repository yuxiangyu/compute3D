...
def las_(fname, version=1.2):
  ...
  #  elif version==2.0:
	# //   return LasVersion2V0(fname).read()
...
class _LasFile(_LasHeader):
    ...
    def read(self):        
        ...
        if self._hdr['sizeofrecord'] != self.recordSizes[self.record_id]:
            ...
            logging.warning('data type implies a record size of %s, but header says %d' % (self.recordSizes[self.record_id],self._hdr['sizeofrecord']))
    ...
    def records(self):  return _LasRcdsIterator(self)
    def close(self):    
    ...
    def save_to(self, rcds, fname,keepVLR=True): # base self config,save to another las, fix it later!!! when rcds.dtype!=self.dtype
    ...
    def make_vlrs(self,name):       raise NotImplementedError
    def make_header(self,rcdid):    raise NotImplementedError
    def _validate_header(self):     raise NotImplementedError
    def _read_more(self):           raise NotImplementedError
